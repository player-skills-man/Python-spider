# YYL->READCODE-> 6
# get api url返回的json数据---API的封装
# 引用并继承自utils.JsonResource类
...
class Schedule(WsResource):
    ...
    def render_POST(self, txrequest):
        #  a post request with data{project= , spider= }
        ...
        # spiders ->a string list
        if not spider in spiders:
            return {"status": "error", "message": "spider '%s' not found" % spider}
        args['settings'] = settings
        jobid = args.pop('jobid', uuid.uuid1().hex)
        args['_job'] = jobid
        # self.root-> <scrapyd.website.Root object...>
        # self.root.scheduler-> <scrapyd.scheduler.SpiderScheduler object...>
...
class Yyltestapi(WsResource):
    def render_GET(self, txrequest):
        return {"node_name": self.root.nodename, "status": "ok", "msg": "yyltest is ok"}
