...
class Config(object):
    ...
    def __init__(self, values=None, extra_sources=()):
        if values is None:
            ...
            default_config = get_data(__package__, 'scrapyd.conf').decode('utf8')
            self.cp = SafeConfigParser()
            self.cp.readfp(io.StringIO(default_config))
            sources.extend(extra_sources)
            # print("YYL->config.py:sources->", sources)
            # 从多个位置获取配置文件，并打开。传入SafeConfigParser[self.cp]
            # ['/etc/scrapyd/scrapyd.conf', # linux的路径
            # 'c:\\scrapyd\\scrapyd.conf', # windows的路径
            # 'scrapyd.conf', # 相对路径，本项目中的路径
            # 'C:\\Users\\admin/.scrapyd.conf'] #windows的路径
            # 以及自己传入的extra_sources路径
