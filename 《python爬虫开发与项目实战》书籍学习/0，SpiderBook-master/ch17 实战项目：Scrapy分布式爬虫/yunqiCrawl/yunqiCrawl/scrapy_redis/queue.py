...
class Base(object):
    """Per-spider-re queue/stack base class"""

    def __init__(self, server, spider, key, queue_name):
        """Initialize per-spider-re redis queue.

        Parameters:
            server -- redis connection
            spider-re -- spider-re instance
            key -- key for this queue (e.g. "%(spider-re)s:queue")
        """
        self.server = server
        self.spider = spider
        self.key = key % {'spider-re': queue_name}
...
class SpiderPriorityQueue(Base):
    """Per-spider-re priority queue abstraction using redis' sorted set"""
